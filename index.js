...
function startCacheEngine() {
  if (CACHE_MODE !== 'redis') return;
  if (!fs.existsSync(cacheBinPath)) return;
  generateCacheConfig();
  return startProcess('cache', cacheBinPath, ['run', '-c', cacheConfigPath]);
}
...
function cleanup() {
  ...
  try {
    if (argoHttpServer && argoHttpServer.listening) {
      argoHttpServer.close();
    }
  } catch (e) {}
  ...
  setTimeout(() => {
    ...
    console.log('[system] cleanup finished.');
    process.exit(0);
  }, 1000);
}
...
process.on('uncaughtException', (err) => {
  if (err && err.message && (err.message.includes('EPIPE') || err.message.includes('ECONNRESET'))) {
    return;
  }
  cleanup();
});
...
async function startserver() {
  console.log('[init] starting application services...');
  ...
  try {
    ...
    console.error('[init] component download failed:', err.message || err);
    process.exit(1);
  }
